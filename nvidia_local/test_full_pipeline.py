...
def test_with_sample_urls():
    ...
    print(f"Model sizes: Depth=vitl (335.3M), DFormerv2=Base (NYU trained), SAM=ViT-H")
    print(f"VRAM strategy: Load model → Process ALL images → Unload")
    print(f"\nExpected VRAM usage:")
    print(f"  - Depth phase (vitl): ~4-6 GB")
    print(f"  - DFormerv2-Base phase: ~8 GB (runs alone after Depth unloads)")
    print(f"  - SAM phase: ~6-7 GB")
    print(f"  - Peak: ~8 GB (within RTX 3060 Ti's 8GB limit, sequential loading)")
    print(f"\nExpected time:")
